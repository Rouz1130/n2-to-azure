...
<div class="ks-item" id="<%=DomID%>">
	<h3><%=Title%><c:ControlDownload runat="server" Name="<%#ControlName%>" /> <a href="<%=ResolveUrl("~/layouts/ComponentTest.aspx?Component=" + ComponentCode + ".1" + (String.IsNullOrEmpty(DefaultChild) ? "" : "&Child=" + DefaultChild))%>" class="small pull-right glyph-link">Layout</a> <a href="#top" class="small pull-right glyph-arrow-up">Top</a></h3>
